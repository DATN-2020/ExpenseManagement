...
class DefaultWalletView(mvpActivity: MvpActivity, viewCreator: AndroidMvpView.ViewCreator) :
    ...
    private val mPresenter = DefaultWalletPresenter(mvpActivity)
    ...
    private val onActionNotify= object : OnActionNotify{
        override fun onActionNotify() {
            ...
            if(amount == 0.0){
                showDialogNotify(title = "Bạn chưa nhập giá trị cho tài khoản")
                return
            }

            if(nameWallet.isNullOrEmpty()){
                showDialogNotify(title = "Bạn chưa nhập tên tài khoản")
                return
            }

            if(idType.getValueOrDefaultIsZero() == 0){
                showDialogNotify(title = "Bạn chưa chọn loại tài khoản")
                return
            }

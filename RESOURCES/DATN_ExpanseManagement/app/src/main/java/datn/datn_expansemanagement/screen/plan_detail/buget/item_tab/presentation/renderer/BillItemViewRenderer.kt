...
class BillItemViewRenderer (context: Context): ViewRenderer<BillItemViewModel>(context){
    ...
    override fun bindView(model: BillItemViewModel, viewRoot: View) {
        GlideImageHelper(context).loadThumbnail(viewRoot.imgTypeBill, model.image, R.drawable.ic_default)
        viewRoot.tvTitleBill.text = model.name
//        viewRoot.tvContentBill.text =
        val day = getDayBetween2Day(model.dateE).toInt()
        when {
            day > 1 -> {
                viewRoot.tvTimeLimitBill.text = "Hết hạn trong ${getDayBetween2Day(model.dateE)} ngày"
            }
            day == 0 -> {
